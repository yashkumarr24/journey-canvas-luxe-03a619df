...
import { hotelApi, useMockHotels } from "@/lib/hotel-api";
 ...
 *
 * The payable amount is read back from the server booking. Nothing here posts
 * an amount, a rate handle or a provider credential: the browser only forwards
 * the provider's acknowledgement so the backend can verify it and then book the
 * hotel.
 */
...
export const Route = createFileRoute("/hotels/checkout")({
  ...
  head: () => ({
    meta: [
      { title: "Secure Hotel Checkout — Fly n Feel Holidays" },
      {
        name: "description",
        content:
          "Check your stay, guests and total, then pay securely to confirm your Fly n Feel Holidays hotel booking.",
      },
      { property: "og:title", content: "Secure Hotel Checkout — Fly n Feel Holidays" },
      { property: "og:description", content: "Pay securely to confirm your hotel booking." },
...
function HotelCheckoutPage() {
  ...
  const confirm = useMutation({
    mutationFn: (payload: { paymentId: string; orderId: string; signature?: string }) => {
      setStage("booking");
      return hotelApi.book({
        bookingReference: ref as string,
        guestToken: guestToken ?? undefined,
        provider: useMockHotels ? "test" : "razorpay",
  ...
  if (!ref) {
    return (
      <Shell>
        <EmptyState title="No booking to pay for" body="Start a new search to choose your stay and guests." />
      </Shell>
    );
  }
      ...
      <Shell>
        <div className="mx-auto max-w-lg rounded-3xl border border-foreground/10 bg-card p-10 text-center">
          ...
          <h2 className="mt-5 font-display text-3xl" aria-live="polite">
            ...
            {stage === "booking" && "Confirming your room…"}
          </h2>
          <p className="mt-3 text-sm text-muted-foreground">
            {stage === "booking"
              ? "We're confirming the room with the hotel. This can take up to a minute — please don't close or refresh this page."
    ...
    <Shell>
      ...
      {expired && (
        <Alert variant="destructive" className="mb-6" role="alert">
          <AlertCircle className="size-4" aria-hidden="true" />
          <AlertTitle>This room is no longer held</AlertTitle>
          <AlertDescription className="space-y-3">
            <p>
              {data.statusMessage ??
                "The hotel released this room before payment was completed. Please search again for current rates."}
            </p>
            <Button asChild size="sm">
          ...
          <div className="rounded-3xl border border-foreground/10 bg-card p-6">
            ...
            {data.expiresAt && (
              <p className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                Room held until {new Date(data.expiresAt).toLocaleString("en-IN")}
              </p>
            )}
            ...
            <p className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
              ...
              Your room is confirmed with the hotel only after payment is verified. If the hotel can't confirm
              it, you're refunded in full.
            </p>
          </div>
        </aside>
      </div>
    ...
    <main className="relative min-h-svh bg-background text-foreground">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 pt-36 sm:pt-40 lg:pt-44">
        <SectionTitle
          ...
          title={
            <>
              One last look, <span className="italic gold-gradient">then it's yours.</span>
            </>
          }
          subtitle="Check the stay, the guests and the total — then pay securely to confirm."
        />
